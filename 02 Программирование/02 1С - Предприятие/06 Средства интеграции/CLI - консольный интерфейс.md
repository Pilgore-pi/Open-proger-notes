...
Открытие клиентского приложения (файловая база)

```batch
..\1cestart.exe ENTERPRISE /S {<ИмяСервераПриложений>\<ИмяБазыДанных>} /N ИмяПользователя /P Пароль
```

Открытие конфигуратора файловой базы

```batch
..\1cestart.exe CONFIG /F <КаталогФайловойБазы> /N ИмяПользователя /P Пароль
```

Открытие конфигуратора файловой базы с последующим открытием внешней обработки

```batch
..\1cestart.exe CONFIG /F <КаталогФайловойБазы> /N ИмяПользователя /P Пароль /Execute <путьКВнешнейОбработке>
```
