> `Nullable<T>` — это надстройка над каким-либо типом, которая позволяет хранить значение `null`, в независимости от того, является ли тип значимым или ссылочным, а для ссылочных типов дает явно понять, что значение `null` допустимо
