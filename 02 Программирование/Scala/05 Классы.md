...
- `object` — синглтон: инициализируется лениво при первом обращении.
- В объекте обычно размещают фабричные методы (`apply`), константы и вспомогательные функции.
- `object` может наследовать классы и trait-ы.

Пример companion + приватный доступ:

```scala
class Secret private(val data: String)
object Secret {
    def create(s: String): Secret = new Secret(s) // может вызывать приватный конструктор
}
```
...
Что ещё дают `case classes`:
- автоматически генерируются `equals`, `hashCode`, `toString`.
- проекция полей — `val` поля по умолчанию.
- поддержка pattern matching через `unapply`.
- `copy` — метод для копирования с изменениями: `p.copy(age = 31)`.
- case classes удобны для immutable data structures.

Пример использования copy и pattern matching:
