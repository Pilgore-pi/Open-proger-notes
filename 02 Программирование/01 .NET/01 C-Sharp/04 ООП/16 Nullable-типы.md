...
---
#MERGE_NOTES
Почему ключевое слово `var` для ссылочных типов всегда выводит nullable-версию?

-- при включенном nullable контексте всегда выводится nullable версия для соблюдения безопасности, так как переменная, объявленная через `var` сейчас может содержать однозначное non-nullable reference type value, но далее в программе эта переменная может принять другое значение, то есть допускается, что в будущем переменная станет `null`
