...
| Атрибут             | Обозначение | Константа в `enum` | Описание                                                                                                                   | Поддержка ОС                                            |
| :------------------ | :---------- | :----------------- | :------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------ |
| `None`              | (нет)       | 0                  | Отсутствие значения (используется только в коде C\#)                                                                       |                                                         |
...
| `System`            | `S`         | 4                  | **Системный** — файл или каталог считается важным для ОС, обычно скрыт от пользователя.                                    | Windows; не поддерживается в Linux/macOS                |
| `Archive`           | `A`         | 16                 | Архивный — указывает, что файл был изменён с момента последнего резервного копирования. Используется резервными системами. | Windows; не поддерживается в Linux/macOS                |
| `Directory`         | (нет)       | 32                 | Каталог (папка) — отличает каталоги от файлов. Не имеет обозначения буквой в attrib.                                       | Windows, Linux, macOS                                   |
