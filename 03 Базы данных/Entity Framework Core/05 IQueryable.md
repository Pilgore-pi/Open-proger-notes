...
Интерфейс `IQueryable` предоставляет дополнительные возможности, которые отсутствуют в `IEnumerable`. Эти возможности связаны с построением и выполнением запросов к удаленным источникам данных. Основное отличие заключается в том, что `IQueryable` позволяет работать с выражениями (`Expression`) и провайдерами запросов (`IQueryProvider`), что делает его мощным инструментом для построения запросов

Специфические свойства `IQueryable`


| Тип              | Свойство     | Описание                                                                                                                                                                                                                |
| ---------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Expression`     | `Expression` | Представляет дерево выражений, описывающее запрос. Используется для анализа и построения запросов. Это свойство позволяет провайдеру запросов (например, Entity Framework) преобразовать запрос в SQL или другой формат |
| `IQueryProvider` | `Provider`   | Провайдер запросов интерпретирует дерево выражений и выполняет запрос к источнику данных                                                                                                                                |

Специфические методы `IQueryable`
...
| `Include<T,TProperty>(this IQueryable<T> src, Expression<Func<T,TProperty>> path)` | `IQueryable<T>` | Соединяет 2 объекта в новый `IQueryable<T>` объект. `Expression` — лямбда-выражение, которое определяет через какие свойства будет построена связь |
...
По умолчанию, доступ к данным через **LINQ-запрос** осуществляется по механизму **ленивой загрузки** (`LazyLoading`). То есть, пока фактического обращения к данным не происходит, запрос выполнен не будет. Это позволяет отложить выполнение запроса до самого последнего момента, когда данные уже необходимы
...
// Здесь выполняется дополнительный запрос к базе данных
...
Если всего хранится 100 заказов, то будет выполнено **101 запрос**:

- 1 запрос для загрузки всех заказов.
- 100 запросов для загрузки данных о клиентах.

Метод `Include()` позволяет использовать **жадную загрузку (eager loading)**, чтобы загрузить связанные данные **вместе с основным объектом** в одном запросе. Это значительно улучшает производительность, так как все данные загружаются за один раз
...
Этот код выполнит **один запрос**, который загрузит данные из таблицы `Orders`, а также связанные данные из таблиц `Customers` и `OrderDetails`

Можно рассматривать метод `Include()` как оператор SQL `join`. Например:
...
- `Customers` — связанная таблица.
- `LEFT JOIN` используется для получения данных из обеих таблиц, даже если у некоторых записей в `Orders` нет связанных записей в `Customers`

#### Различия JOIN и Include()


|                 | SQL JOIN                                                                      | Include()                                                                                                                                          |
| --------------- | ----------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Назначение      | Используется для объединения данных из двух или более таблиц в одном запросе  | Используется для загрузки связанных данных в виде объектов                                                                                         |
| Результат       | "Плоская" таблица, где строки содержат данные из всех объединенных таблиц     | объектная модель, где связанные данные загружаются в навигационные свойства                                                                        |
| Тип связи       | Можно явно указать (`LEFT`, `RIGHT`, `INNER`, `OUTER`...)                     | Entity Framework всегда использует `LEFT JOIN`, чтобы гарантировать, что основная сущность будет загружена, даже если связанные данные отсутствуют |
| Выбираемые поля | Явно указываются                                                              | По умолчанию все поля. Чтобы выбрать конкретные поля, нужно создать проекцию данных через `Select()`                                               |
| Связь 1 : М     | Каждая строка таблицы источника будет дублироваться для присоединяемой записи | Entity Framework автоматически группирует связанные данные в коллекции                                                                             |

Главное отличие SQL JOIN и `Include()` в том, что JOIN возвращает набор атомарных полей, а `Include()` возвращает сложные объекты, которые могут иметь вложенные объекты
...
- **`IQueryable`**, если:
    ...
    - Есть необходимость в построении сложных запросов с фильтрацией, сортировкой и т.д.
