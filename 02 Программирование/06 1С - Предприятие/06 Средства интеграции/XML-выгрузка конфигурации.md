...
### Преобразование объектов 1С

`.cf` может быть преобразован в `.cfe` и наоборот. Для этого достаточно выгрузить `.cf` в файлы XML и изменить блок `<Properties>` так, чтобы он соответствовал блоку `<Properties>` из `.cfe` XML-выгрузки.

Обработки и отчеты можно сохранить как внешние объекты. `.epf` - внешняя обработка, `.erf` - внешний отчет. Далее эти объекты можно выгрузить в файлы XML. Обратно загрузить внешние объекты также можно.
