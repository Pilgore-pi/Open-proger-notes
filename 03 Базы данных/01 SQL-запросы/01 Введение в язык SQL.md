...
SQL — это общий стандарт, на основе которого были разработаны диалекты — версии языка SQL с небольшими изменениями и дополнительными возможностями

## Механизм запросов

Запрос создает пользователь. Запрос интерпретируется **системой управления базами данных (СУБД)** в низкоуровневые машинные команды. Современные СУБД применяют множество различных оптимизаций для достижения максимальной производительности при работе с базами данных

Путь запроса от пользователя к клиенту:

1. **Пользователь**: формирует запрос к базе данных
2. **Клиент**: графический интерфейс для ввода запроса пользователя в компьютер. Клиент генерирует SQL команды для передачи их СУБД
3. **СУБД**: интерпритирует команды SQL, сформированные Клиентом
4. **База данных**: система хранения таблиц, связанных между собой

Операторы языка SQL делятся на следующие категории:

## [DDL](01%20Язык%20DDL.md) (Data Definition Language)
...
## [DML](01%20Язык%20DML.md) (Data Manupilation Language)
...
> ** [Транзакции](01%20Введение%20в%20транзакции.md)** — последовательности логически связанных SQL-запросов, рассматриваемых как одно целое. Если все команды выполнены успешно, транзакция завершается, в противном случае откатывается назад.
...
| Тип литерала x Диалект | MS SQL Server | MySQL | PostreSQL | SQL Lite | Oracle, PL-SQL |
| ---------------------- | ------------- | ----- | --------- | -------- | -------------- |
| Строка                 | `'string'`    |       |           |          |                |
