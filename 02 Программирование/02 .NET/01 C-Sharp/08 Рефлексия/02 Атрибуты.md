> **Атрибут** — это метаданные, связанные с какой-либо структурной единицей кода (класс, метод, свойство, поле, перечисление и т. д.). Атрибуты в языке C\# все являются классами из семейства `Attribute`

Все атрибуты находятся в семействе System.Attrubute.
Атрибуты бывают:

- **Встроенные**
- **Пользовательские**

В атрибуте можно указать мета информацию, которую затем можно будет получить в другой области кода через специальный метод `Attribute.GetCustomAttribute()`
...
При создании атрибута, в конце имени класса всегда нужно добавлять "Attribute". Далее при использовании пользовательского атрибута можно опускать суффикс `Attribute` или оставлять его — это опционально
