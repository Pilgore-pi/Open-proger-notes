...
- Узел может содержать более двух потомков
- Все листья находятся на одном уровне
- Узел содержит от t-1 до 2t-1 ключей (t — минимальная степень)
...
> **Trie (префиксное дерево)** — дерево для хранения строк, где путь от корня до узла представляет префикс
