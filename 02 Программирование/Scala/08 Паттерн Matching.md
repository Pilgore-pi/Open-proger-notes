...
олезные приёмы и замечания:
- Порядок case имеет значение — первый подходящий case выполняется.
- Если trait/классы помечены `sealed`, компилятор может проверить exhaustiveness (покрытие всех случаев).
- Проверка типа `case x: List[Int]` может быть неточной из-за type erasure — компилятор предупредит о небезопасном касте.
