...
Оповещение о завершении происходит при помощи вызова процедуры `OnCompleted()`, описание которой передается в `AsyncProc()`. Эта процедура и называется методом обратного вызова (англ. callback – дословно “обратный звонок”), поскольку позволяет асинхронному методу `AsyncProc()` “связаться” с вызвавшим его кодом: