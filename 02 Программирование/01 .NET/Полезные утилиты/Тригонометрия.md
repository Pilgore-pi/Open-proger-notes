# Ну не практично же! Изменить содержимое этого и других файлов, объединив все блоки с кодом в один статический класс
...
```csharp
static (double, double) ToCartesian(double r, double phi) =>
(
    r * Math.Cos(phi),
    r * Math.Sin(phi)
);
...
```csharp
static (double, double) ToPolar(double x, double y) =>
(
    Math.Sqrt(x * x + y * y),
    Math.Atan2(y, x)
);
...
```csharp
static double GetVectorAngle(double x, double y) => x switch
{
    > 0 => Atan(y / x),
    < 0 => PI + Atan(y / x),
    0 => Asin(y),
...
```csharp
static double GetVectorAngle(double x, double y)
{
