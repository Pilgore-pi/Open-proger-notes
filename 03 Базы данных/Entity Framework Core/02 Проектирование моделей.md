...
| `[Timestamp]` | Свойство, поле | Указывает на то, что поле данных является уникальным бинарным числом, обозначающее версию всей записи |
| `[DeniedValues(object[])]` | Свойство, поле, параметр | Определяет список недопустимых значений для поля данных |
| `[AllowedValues(object[])]` | Свойство, поле, параметр | Определяет список разрешенных значений для поля данных |
| `[Range(min, max)]` | Свойство, поле, параметр | Определяет диапазон допустимых значений (как числовых, так и специфичных для данного типа данных (перегрузка `Range(Type, String, String)`)) |
...
| `[Length(int min, int max)]` | Свойство, поле, параметр | Указывает минимальную и максимальную длину для коллекции или строки (объединяет 2 атрибута `MinLength` и `MaxLength` |
| `[StringLength(length)]` | Свойство, поле, параметр | Контролирует, чтобы поле данных было указанной длины в символах |
...
| `[Index(propNames)]` | Класс | Объявляет индекс БД для таблицы SQL. Индекс может быть простым (1 поле даных) или составным (несколько полей данных) |
...
| `[Url]` | Свойство, поле, параметр | Проверяет, начинается ли значение на `http://`, `https://`, `ftp://` |
| `[FileExtension]` | Свойство, поле, параметр | Проверяет, соответствует ли поле данных одному из указанных расширений файлов. Если расширений не указано, используется значение по умолчанию: `"png,jpg,jpeg,gif"` |
| `[Phone]` | Свойство, поле, параметр | Проверяет, соответствует ли поле данных формату номера телефона |
| `[CreditCard]` | Свойство, поле, параметр | Проверяет, соответствует ли поле данных формату номера кредитной карты |
...
### Обязательные и необязательные поля данных

https://learn.microsoft.com/en-us/ef/core/modeling/entity-properties?tabs=data-annotations%2Cwith-nrt#required-and-optional-properties
https://learn.microsoft.com/en-us/ef/core/miscellaneous/nullable-reference-types#required-and-optional-properties
...
Чтобы определить свой механизм валидации данных можно воспользоваться одним из двух атрибутов:

- `CustomValidationAttibute`: позволяет указать метод валидации, в котором определяется логика проверки
- Создать собственный валидатор, наследуемый от `ValidationAttribute`: более гибкий способ

Пример использования `CustomValidationAttibute` для проверки строкового значения:

```cs
using System.ComponentModel.DataAnnotations;

public class MyClass {
    [CustomValidation(typeof(MyClass), nameof(Validate))]
...
Пример использования собственной реализации `ValidationAttribute` (Ограничение длины строки):
