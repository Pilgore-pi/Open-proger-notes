...
Задание может быть запущено по одному или множеству триггеров. Триггеры бывают следующих видов:

- По расписанию:
    - В определенное время (однократно)
    - Ежедневно
    - Еженедельно
    - Ежемесячно
- При запуске системы
...
- **При создании или изменении задачи**: задача активируется, когда она создается или любые ее параметры и параметры ее триггеров редактируются
- При блокировании рабочей станции
- При разблокировании рабочей станции

Для триггеров можно задавать дополнительные параметры. Доступность доп. параметров зависит от вида триггера.

- Временная задержка перед запуском задачи
- Повтор выполнения задачи через заданный интервал времени
- Установка конечной временной границы, по достижении которой задача повтор задачи, более, не выполняется

### Журнал регистрации задач

В журнале регистрации ведутся логи задач со всеми происходящими событиями (запуск, регистрация задачи, изменение, ошибки)
...
### Управление через командную оболочку

Чтобы **запустить Планировщик заданий из .bat файла**, можно вызвать системную утилиту `taskschd.msc`, которая открывает графический интерфейс Планировщика заданий. В .bat файле это делается так:

```bat
start taskschd.msc
```

Если же нужно **создать или запустить конкретную задачу планировщика из .bat скрипта**, то для этого используется команда `schtasks`.

Пример запуска уже созданной задачи с именем "MyTask":

```bat
schtasks /run /tn "MyTask"
```

Пример создания задачи, которая запускает bat-файл каждый день в 12:00:

```bat
schtasks /create /tn "MyDailyTask" /tr "C:\Путь\к\скрипту.bat" /sc daily /st 12:00
```

Таким образом, в зависимости от задачи:

- Для **открытия интерфейса Планировщика задач** из .bat файла:

```bat
start taskschd.msc
```

- Для **запуска существующей задачи**:

```bat
schtasks /run /tn "ИмяЗадачи"
```

- Для **создания новой задачи** с расписанием:

```bat
schtasks /create /tn "ИмяЗадачи" /tr "ПутьКФайлу.bat" /sc daily /st 12:00
```
