...
>Определение 2 (более узкое): **_Класс_** — это пользовательский тип данных

**_Объект_** — это экземпляр шаблона (класса, структуры или интерфейса)
**_Структура_** — это упрощенная версия класса
...
| Индексатор (механизм доступа к данным через оператор индексирования)                               | да           | да            | да               |
| Событие                                                                                            | да           | да            | да               |
...
| Модификатор доступа  | Описание                                                                                                                                                            | Применяемость                                                         |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `public`             | Доступность из любого места в коде                                                                                                                                  | class, struct, interface, field, property (get, set тоже), operator,  |
...
Для каждого свойства **всегда** определяется соответствующее поле. Поле может определяться явно или скрыто компилятором
