...
Nullable-контекст позволяет компилятору на этапе компиляции программы определить, какие переменные допускают значение null и создать предупреждение в среде разработки.
