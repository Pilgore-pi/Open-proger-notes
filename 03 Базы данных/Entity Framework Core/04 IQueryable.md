...
По умолчанию, доступ к данным через **LINQ-запрос** осуществляется по механизму **ленивой загрузки** (`LazyLoading`). То есть, пока фактического обращения к данным не происходит, запрос выполнен не будет. 
