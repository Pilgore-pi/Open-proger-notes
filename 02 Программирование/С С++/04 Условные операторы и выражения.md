...
> В C++ условные операторы `if-else` и `switch-case` являются **операторами**, а не выражениями. Однако, начиная с C++17, появился оператор `if constexpr` для вычисления условий на этапе компиляции. Тернарный условный оператор `?:` является **выражением**, возвращающим одно из двух значений.
...
```cpp
// Переменная в switch должна быть целочисленного типа или типа enum
switch (static_cast<int>(value)) {
    case 1: /* ... */ break;
}

// Инициализация переменной в switch (C++17)
