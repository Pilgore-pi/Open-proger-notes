...
Схема работы модели TCP / IP (ЗАМЕНИТЬ SVG картинкой):

```text
        Отправление сообщения (последовательное оборачивание заголовками)
                |
Прикладной      | Сообщение
                |
Транспортный    | Сообщение + TCP заголовок
                |
Сетевой         | Сообщение + TCP заг. + IP заг.
                |
Канальный       | Сообщение + TCP заг. + IP заг. + Канальный заг.
                |
                v

        Получение сообщения (последовательное разворачивание)
                ^
                |
Прикладной      | Сообщение
                |
...
Согласно протоколу, сформированные на предыдущем этапе сегменты с заголовками TCP, UDP или др. ~~дополняются IP-заголовком~~ (В КАКОМ НАПРАВЛЕНИИ СОБИРАЮТСЯ ЗАГОЛОВКИ СООБЩЕНИЯ???). После этой операции **сегмент становится *пакетом***
...
> Максимальный размер пакета данных, передаваемого на сетевом уровне называется термином MTU (Maximum Transmission Unit)
