...
Процедура НачатьПроверкуЛицензииИПодключениеКВнешнейСистеме(
    ...
    Если ПроверитьЛицензию() Тогда
        ВыполнитьОбработкуОповещения(ОбработкаПродолжения, Истина); // (3.a)
    Иначе
        ПоказатьВопросВводаЛицензии(ОбработкаПродолжения);          // (3.b)
    КонецЕсли;
    
КонецПроцедуры

&НаКлиенте // (3.a) Callback
...
&НаСервереБезКонтекста // Синхронный
Функция ПроверитьЛицензию()
