...
Назад: [[04 НАСЛЕДОВАНИЕ|Наследование]]

Далее: [[06 Статические классы|Статические классы]]

# В общих чертах описать различия типов значений и ссылочных, ValueType и описать структуру наследования корневых типов
...
> Применять механизм упаковки следует только, если это необходимо
...
`dateTime` — переменная значимого типа (`DateTime`). У `dateTime` вызывается метод `GetType`, определённый в типе `System.Object`. Чтобы выполнить такой вызов, объект dateTime придётся упаковать

### Nullable-типы

Операции упаковки и распаковки поддерживают [Nullable-типы](16%20Nullable-типы.md)
