...
| Логика *"перед каждым тестом"* и *"после каждого теста"* | `[SetUp]`, `[TearDown]`                    | Специальный конструктор (перед), `IDisposable.Dispose()` (после) | `[TestInitialize]`, `[TestCleanup]`   |
...
Для использования шаблона проекта xUnit достаточно выполнить однку команду в консоли:
