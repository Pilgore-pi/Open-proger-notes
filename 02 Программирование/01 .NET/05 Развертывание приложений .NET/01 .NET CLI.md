...
## Что такое профиль публикации?

- Файл с расширением `.pubxml`, который хранится в папке проекта `Properties/PublishProfiles/`.
    
- Содержит настройки публикации: целевой объект (папка, Azure, IIS и др.), конфигурацию сборки, путь вывода, параметры runtime и прочее.
    
- Может содержать специфичные для среды данные, например, идентификаторы подписок Azure.
    
- Конфиденциальные данные (пароли и т.п.) хранятся отдельно в файлах `.pubxml.user` и обычно не добавляются в систему контроля версий.
...
- Можно запускать публикацию из Visual Studio или из командной строки, указывая профиль.
