...
## Атрибуты целостности сопоставляемых полей

Все атрибуты целостности определены в пространства имен `System.ComponentModel.DataAnnotations` и `System.ComponentModel.DataAnnotations.Schema`
...
|--------------------|-------------------|----------|
| `[NotMapped]` | Класс, cвойство, поле | Исключает свойство из модели данных EF Core |
| `[Table(name)]` | Класс | Задает имя соответствующей SQL таблицы |
| `[Column(propName)]` | Свойство, поле, параметр | Явно задает сопоставляемое имя атрибута SQL |
| `[Column(Order = index)]` | Свойство, поле | Задает порядок атрибута в таблице SQL |
| `[Key]` | Свойство, поле, параметр | Объявляет текущее свойство как первичный ключ |
| `[PrimaryKey(propertyList)]` | Свойство, поле | Объявляет первичный ключ для объекта. В атрибут передаются все имена свойств, формирующих составной или простой первичный ключ |
...
| `[Index(propName)]` (устаревший) | Класс | Объявляет индекс БД. Индекс может быть простым (1 свойство) или составным (более 1 свойства)  |
...
| `[Unicode(flag)]` | Свойство, поле (`string`) | Определяет, будет ли применяться кодировка Unicode к текстовому свойству |
| `[Comment(commentString)]` | Класс, свойство, поле | Задает комментарий для колонки таблицы SQL на уровне схемы |
...
| `[EmailAddress]` | Свойство, поле, параметр | Помечает свойство для простой проверки на соответствие формату e-mail адреса (проверяется, является ли символ `@` единственным и непоследним в строке) |
