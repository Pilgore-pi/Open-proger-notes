...
> **Инкапсуляция** —  это механизм, суть которого заключается в объединении данные и методы работы с ними в единую структуру (класс), скрывая детали реализации и защищая данные от некорректного использования
...
C# предоставляет несколько модификаторов доступа для управления видимостью членов класса:

| Модификатор | Описание | Доступность |
|------------|----------|-------------|
| `public` | Публичный доступ | Везде |
| `private` | Приватный доступ | Только внутри класса |
| `protected` | Защищённый доступ | Внутри класса и производных классов |
| `internal` | Внутренний доступ | Внутри текущей сборки |
| `protected internal` | Комбинированный | Внутри сборки ИЛИ в производных классах |
| `private protected` | Комбинированный | Внутри сборки И в производных классах |
...
```csharp
public class Person {
    // Плохо: публичное поле
    // public string name;
    
    // Хорошо: приватное поле с публичным свойством
    private string name;
