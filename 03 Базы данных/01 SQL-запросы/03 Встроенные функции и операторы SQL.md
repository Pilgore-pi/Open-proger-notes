...
| - | вычитание | 2 - 3 | -1 |
| * | умножение | 2 * 3 | 6 |
| / | деление (деление целого отсекает дробную часть) | 4 / 2 | 2 |
| % | остаток от деления | 5 % 4 | 1 |
| ^ | возведение в степень | 2.0 ^ 3.0 | 8 |
| \|/ | квадратный корень | \|/ 25.0 | 5 |
| \|\|/ | кубический корень | \|\|/ 27.0 | 3 |
| ! | факториал (**устарел**, вместо этого следует использовать `factorial()`) | 5 ! | 120 |
| !! | факториал как префиксный оператор (**устарел**, вместо этого следует использовать `factorial()`) | !! 5 | 120 |
| @ | число без знака | @ -5.0 | 5 |
| & | побитовое И | 91 & 15 | 11 |
| \| | побитовое ИЛИ | 32 \| 3 | 35 |
| \# | побитовое исключающее ИЛИ | 17 # 5 | 20 |
| ~ | побитвое отрицание | ~1 | -2 |
| << | поразрядный сдвиг влево | 1 << 4 | 16 |
| >> | поразрядный сдвиг вправо | 8 >> 2 | 2 |
...
| LOG(base, number)  | Логарифм по произвольному основанию `base`               |
...
| `format(formatstr text [, formatarg "any" [, ...] ])` | `text` | Format arguments according to a format string. This function is similar to the C function sprintf | `format('Hello %s, %1$s', 'World')` | Hello World, World |
...
| Функция                | Возвращаемый тип | Описание |
| ---------------------- | ---------------- | -------- |
| NOW()                  | `timestamp` (Postgres) | (`GETDATE()` в MS SQL) Возвращает текущую дату и время |
...
| YEAR(date)             | `int` | возвращает год указанной даты типа date |
| MONTH(date)            | `int` | месяц указанной даты типа date |
| DAY(date)              | `int` | день указанной даты типа date |
