> **Roslyn** — это современная **платформа компилятора .NET** с открытым исходным кодом, которая включает в себя компиляторы C# и Visual Basic, а также мощные API для анализа, разбора и динамической компиляции кода

Существует статья от ведущего архитектора C# по Roslyn, переведенная на русский язык, в которой можно узнать историю создания платформы 
