...
***Когда объявлять ссылочный тип как nullable?*** — Когда данный ссылочный тип может принимать значение `null` и эта ситуация учитываются в коде. Если ссылочный тип не предполагает то, что он будет иметь значение `null`, то не следует объявлять его как nullable. Например, тип не должен быть nullable, если он может при
