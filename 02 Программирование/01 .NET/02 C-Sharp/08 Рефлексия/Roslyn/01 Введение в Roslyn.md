> **Roslyn** — это современная реализация компиляторов C\# и Visual Basic**платформа компилятора .NET** с [открытым исходным кодом](https://github.com/dotnet/roslyn), которая включает в себя компиляторы C# и Visual Basic, а также мощные API для анализа, разбора и динамической компиляции кода
