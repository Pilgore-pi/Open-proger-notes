...
EF Core поддерживает множество релиционных и нереляционных СУБД:
...
> **POCO классы (Plain Old CLR Object)** -- это простые классы .NET, которые не зависят от каких-либо внешних библиотек или фреймворков и не наследуются от специальных базовых классов. Их задача — просто описывать структуру данных, обычно с помощью автосвойств, без бизнес-логики, аннотаций или специфических интерфейсов
...
> Для взаимодействия с базой данных через EF Core, принято исполоьзовать язык **LINQ to entities**. [Интерфейс IQueryable]() позволяет создавать оптимизированные запросы для работы с данными в удаленной базе.
